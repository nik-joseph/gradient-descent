...
def get_report(cpu_func, gpu_func, title, matrix_sizes=(10, 1000, 10)):
    cpu_report = [
        timeit(cpu_func, *np.random.randint(matrix_size, size=[2, matrix_size, matrix_size]))
        for matrix_size in trange(*matrix_sizes)
    ]
    gpu_report = [
        timeit(gpu_func, *np.random.randint(matrix_size, size=[2, matrix_size, matrix_size]))
        for matrix_size in trange(*matrix_sizes)
    ]
